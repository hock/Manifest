...
function viz_cleanup() {
	...
	if(MI.scview.active_point) { MI.scview.active_point.closePopup(); MI.scview.active_point = null; }
}
...
function viz_forcegraph(graph, id) {	
	...
	var adj_width = width / 2;		
	if(width > 920) {
		if($("body").hasClass("fullscreen")) {
			adj_width = width / 2;
		} else {
			adj_width = $(".sidepanel").width() / 2 + width / 2;
		}
	}  else {
		height = 600;
	}
	...
	var simulation = d3.forceSimulation()
	    .velocityDecay(0.5)
	    .force("x", d3.forceX(adj_width).strength(0.1))
