...
function SpatialSupplyChain() {
	this.map = new L.Map('map', { worldCopyJump: true, center : new L.LatLng(40,-100), zoom : 3 });
	this.clustergroup;
...
function SourcemapAPI(id, map, layerstyle) {
	$.getJSON(ManifestInstance.resourceurl + id + ".geojson", function(d) {
		if (typeof(d.properties.title) != 'undefined' && $("#mtitle").text().length == 1) {
			document.title = d.properties.title + " - Manifest";
			$("#mtitle").html('<a target="_blank" href="">' + d.properties.title + '</a>');
			$(".titlepanel").height($("#mtitle").height()); $("#mdescription").css("paddingTop", $("#mtitle").height()+60);
		}
		if (typeof(d.properties.description) != 'undefined' && $("#mdescription").text().length == 1) { $("#mdescription").append("<p>"+d.properties.description+"</p>"); }
		for (i in d.features) {
		...
		pointLayer.on('click', function(e){
			$('.sidepanel').scrollTo($("li#local_"+e.layer._popup._source.feature.properties.lid),  500, { offset:{ top:-1*($("#mtitle").outerHeight()),left:0 }});
		});
...
function Cleanup() {
	if(ManifestInstance.attributes.waypointscroll) {
		$('#mlist li').waypoint(function() {
			ManifestInstance.ssc.focus($(this).attr("id").substring(6));
		}, { context: '.sidepanel', offset: -1 * $(".titlepanel").height()+$("#mdescription").height(), continuous: false });
	}
