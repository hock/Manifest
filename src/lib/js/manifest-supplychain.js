						...
						<div id="mdetails-${id}" class="mdetails">
						    <div id="share-options-${id}" class="share-container closed">
							    <ul class="share-menu" style="color:${manifest.details.style.fillColor}; background:${manifest.details.style.lightColor};">
									<span>Share:</span>
							        <li><span class="share-link" onclick="window.open('${ManifestUtilities.Slugify(manifest.details.url)}', '_blank');">Open</span> <i class="fas fa-window-restore"></i></li>
							        <li><span class="share-link" onclick="MI.ExportManifest(${id}, ${id}, 'embed');">Embed</span> <i class="fas fa-window-restore"></i></li>		
							        <li><span class="share-link" onclick="MI.ExportManifest(${id}, ${id}, 'markdown');">Markdown</span> <i class="fas fa-file-download"></i></li>
							        <li><span class="share-link" onclick="MI.ExportManifest(${id}, ${id}, 'json');">JSON</span> <i class="fas fa-file-download"></i></li>
							    </ul>
							</div>
	...
	SetupPoint(ft, d, index) {
		...
		`<li id="node_${ft.properties.lid}" class="mnode" data-scref="${d.details.id}" tabindex="0">
			...
			${ft.properties.images.length !== 0 ? `<div class="node-images ${ft.properties.images.length > 1 ? `multiple` : ''}" ${ft.properties.images.length > 1 ? `data-index="1"` : ''}>
				...
				`<img class="ftimg" loading="${MI.Interface.LoadingImages(i) ? 'auto' : 'lazy'}" src="${img.URL}" data-caption="${img.caption ? 'true' : 'false'}" title="${img.caption ? img.caption : ft.properties.title}" alt="${img.caption ? img.caption : ft.properties.title}${img.caption ? '' : ` image`}"/>` ) : '').join('')} 	
			</div>` : ''}
